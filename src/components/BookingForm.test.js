...
const renderBookingForm = (overrideProps = {}) => {
  ...
  return props; // testlərdə submitForm və s. lazım ola bilər
};
...
describe('BookingForm – JavaScript validation & submission', () => {
  test('shows error and disables submit when guests < 1', () => {
    ...
    // Tarix və saatı dolduraq ki, yalnız guests səhv olsun
    fireEvent.change(dateInput, { target: { value: today } });
    fireEvent.change(timeSelect, { target: { value: '17:00' } });

    // Guests = 0 (səhv)
    fireEvent.change(guestsInput, { target: { value: '0' } });
    fireEvent.blur(guestsInput); // touched = true olsun ki, error görünsün
    ...
    // Form invalid olduğuna görə submit disabled olmalıdır
  ...
  test('enables submit and calls submitForm with correct data when form is valid', () => {
    ...
    // Bütün field-ları düzgün dolduraq
    fireEvent.change(dateInput, { target: { value: today } });
    fireEvent.change(timeSelect, { target: { value: '17:00' } });
    fireEvent.change(guestsInput, { target: { value: '4' } });

    // Artıq form valid olmalıdır
    ...
    expect(submitForm).toHaveBeenCalledWith({
      ...
      time: '17:00',
      guests: 4,
      occasion: 'Birthday', // default state
    });
  });
});